    ...
    const Eigen::Matrix<double, 6, 6>& robotPoseCovariance, const ros::Time& time)
{
 ...
 // if (((robotPoseCovarianceScaled - RCovariance_).array() == 0.0).all()) return false;
  ...
  try{
      listener.waitForTransform("/camera_depth_frame", "/base_link",ros::Time(0), ros::Duration(1.0));
      listener.lookupTransform("/camera_depth_frame", "/base_link", ros::Time(0), transform1);
      listener.waitForTransform("/odom", "/camera_depth_frame", ros::Time(0), ros::Duration(1.0));
      listener.lookupTransform("/odom", "/camera_depth_frame",ros::Time(0), transform2);
    }
