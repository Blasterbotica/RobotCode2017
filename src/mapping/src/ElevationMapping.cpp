      ...
      ignoreRobotMotionUpdates_(false)
{
  ...
  //ros::NodeHandle n;
  pointCloudSubscriber_ = nodeHandle_.subscribe("/camera/depth/points", 1, &ElevationMapping::pointCloudCallback, this);
...
bool ElevationMapping::readParameters()
{
 ...
 // nodeHandle_.param("point_cloud_topic", pointCloudTopic_, string("camera/depth_regristered/points"));
  ...
  string frameId;
  nodeHandle_.param("map_frame_id", frameId, string("/odom"));
  map_.setFrameId(frameId);

  grid_map::Length length;
  grid_map::Position position;
  double resolution;
  nodeHandle_.param("length_in_x", length(0), 4.0);
  nodeHandle_.param("length_in_y", length(1), 4.0);
  nodeHandle_.param("position_x", position.x(), 0.0);
  nodeHandle_.param("position_y", position.y(), 0.0);
  nodeHandle_.param("resolution", resolution, 0.60);
...
bool ElevationMapping::updatePrediction(const ros::Time& time)
{
 ...
 boost::shared_ptr<nav_msgs::Odometry const> poseMessage = robotPoseCache_.getElemBeforeTime(time);
  ...
  try{
      ...
      listener.lookupTransform("/odom", "/base_link", ros::Time(0), tran3);
   }
...
bool ElevationMapping::updateMapLocation()
{
  ...
  trackPoint.point.x = trackPoint_(1);
  trackPoint.point.x = trackPoint_(2);
